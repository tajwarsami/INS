# checkpoint2_cipher2.py
# Automatic substitution cipher solver for Cipher 2
# Usage: python checkpoint2_cipher2.py cipher2.txt
...
def read_file(path):
    with open(path,'r') as f: return f.read().upper()
...
def solve_substitution(cipher):
    ...
    for i in range(8000):
        ...
        candidate_score=frequency_score(apply_key(cipher,candidate))
...
if __name__=="__main__":
    if len(sys.argv)<2:
        print("Usage: python checkpoint2_cipher2.py cipher2.txt")
        sys.exit()

    cipher_text=read_file(sys.argv[1])
    key,plaintext=solve_substitution(cipher_text)
    print("---- DECRYPTED TEXT (Cipher 2) ----\n")
    print(plaintext)
    print("\n---- KEY MAPPING ----")
